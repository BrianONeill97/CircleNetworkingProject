...
bool Server::SendCircle(int ID, std::string& _string)
{
	if (!SendPacketType(ID, P_CircleData)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	int bufferlength = _string.size(); //Find string buffer length
...
bool Server::GetCircle(int ID, std::string& _string)
{
	not_digit notADigit;
	int bufferlength; //Holds length of the message
	if (!GetInt(ID, bufferlength)) //Get length of buffer and store it in variable: bufferlength
...
bool Server::getEndGame(int ID, bool& endgame)
{
	return false;
}

bool Server::sendEndGame(int ID, bool& endgame)
{
	return false;
}
