...
bool Server::ProcessPacket(int ID, Packet _packettype)
{
	...
	{
		std::string Message; //string to store our message we received
		if (!GetCircle(ID, Message)) //Get the chat message and store it in variable: Message
			return false; //If we do not properly get the chat message, return false
		////Next we need to send the message out to each user
	...
	//	for (int i = 0; i < TotalConnections; i++)
	//	{
	//		if (i == ID) // if connection is to user who sent the message
	//			continue; //skip to next  user
	//		if (!sendEndGame(i, *endgame))
	//		{
	//			std::cout << "Failed to send data from endgame" << std::endl;
	//		}
	//	}
	//}
